<template>
  <BaseDetailModal
    ...
    :api-detail-fn="getDetailNews"
    id-key="idberita" @close="$emit('close')"
  >
    <template #detail="{ item }">
      
      <div class= "text-center mb-4">
        <img
          v-if="item.gambar_utama" :src="item.gambar_utama" alt="Gambar Utama Berita" class="foto"
        ...
        <h2 class="nama-barang mt-3">
          {{ item.judul || 'Judul Berita' }} </h2>
        <h5 class="nama-barang-en text-muted fw-normal">
          {{ item.judul_en || 'Judul English' }} </h5>
        <p class="merk">{{ item['news-categories']?.namakategoriberita || 'Kategori tidak tersedia' }}</p>
      </div>

      <hr>

      <div class="detail-section">
        <h4><i class="fa fa-info-circle me-2"></i>Detail Publikasi & Status</h4> <div class="row detail-grid-container">
          <div class="col-md-6">
      ...
      <div class="detail-section">
        <h4><i class="fa fa-file-text-o me-2"></i>Konten Berita</h4> <div class="row">
          <div class="mb-3">
            <h5>Konten (ID)</h5>
...
import { getDetailNews } from '@/services/general/website/news/news';
...
.text-content {
  /* white-space: pre-wrap; DIHAPUS agar v-html berfungsi normal */
