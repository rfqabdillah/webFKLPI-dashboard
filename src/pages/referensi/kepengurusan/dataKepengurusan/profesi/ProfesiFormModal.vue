<template>
  ...
  >
    ...
    >
      <div class="mb-3">
        <label class="form-label fw-semibold">Kode Profesi</label>
        <input
          type="text"
          class="form-control"
          ...
          :class="{
            ...
            'is-valid': !formErrors.kode_kbji && wasValidated,
          }"
          placeholder="Masukkan kode profesi"
          required
        />
      ...
      <div class="mb-3">
        ...
        <input
          type="text"
          class="form-control"
          v-model="formData.nama_kbji"
          :class="{
            'is-invalid': formErrors.nama_kbji,
            shake: isShaking && formErrors.nama_kbji,
            'is-valid': !formErrors.nama_kbji && wasValidated,
          }"
          placeholder="Masukkan nama profesi"
          required
        />
        <transition name="fade-error">
          <div class="invalid-feedback" v-if="formErrors.nama_kbji">
            {{ formErrors.nama_kbji }}
          </div>
        </transition>
      </div>
...
import { addKbji, updateKbji } from "@/services/referensi/kbji";
...
const validationSchema = yup.object().shape({
  nama_kbji: yup.string().required("Nama Profesi wajib diisi."),
  kode_kbji: yup.string().required("Kode KBJI wajib diisi."),
});
...
watch(
  ...
  (newData) => {
    if (newData) {
      ...
      formData.nama_kbji = newData.nama_kbji;
    } else {
      formData.kode_kbji = "";
      formData.nama_kbji = "";
    }
...
async function submitForm() {
  ...
  data.append("record[nama_kbji]", formData.nama_kbji);

  try {
    if (isEditMode.value) {
      const fieldId = props.fieldToEdit.kode_kbji;
      data.append("_method", "PUT");
      await updateKbji(fieldId, data);
