...
import {
  ...
  updateMenu,
  deleteMenu,
} from "@/services/general/website/menu";

// === Komponen & State ===
const MenuFormModal = defineAsyncComponent(() => import("./MenuFormModal.vue"));
...
function buildTree(items, parentId = null, visited = new Set()) {
  const tree = [];
  
  const isRootLevel = parentId === null || parentId === "" || parentId === 0 || parentId === "0";
  
  const children = items
    .filter((item) => {
      const parentOfItem = item.induk === null || item.induk === "" || item.induk === 0 || item.induk === "0" ? null : String(item.induk);
      
      if (isRootLevel) {
        return parentOfItem === null;
      }
      return parentOfItem === String(parentId);
    })
    .sort((a, b) => {
  ...
  for (const item of children) {
    if (!item.id_menu || visited.has(item.id_menu) || String(item.id_menu) === String(parentId)) {
      continue;
    }
    
    visited.add(item.id_menu);

    const grandchildren = buildTree(items, item.id_menu, visited);
...
function flattenTree(items, parentId = null) {
  ...
  items.forEach((item, index) => {
    ...
    if (item.children && item.children.length > 0) {
      flatList = flatList.concat(flattenTree(item.children, item.id_menu));
    }
  });
  return flatList;
}
...
async function handleSaveStructure() {
  ...
  try {
    ...
    const updatePromises = flattenedData.map((item) => {
      ...
      return updateMenu(item.id_menu, data);
    });
