        ...
        <div class="col-md-6">
          ...
          <div class="mb-3">
            ...
            <input
              ...
              :class="{
                'is-invalid': formErrors.gambarutama,
                shake: isShaking && formErrors.gambarutama,
              }"
              ref="fileInput"
            />
            <div class="form-text small">
              ...
              <span
                v-if="isEditMode && fieldToEdit && fieldToEdit.gambarutama"
                class="d-block"
              >
            ...
            <transition name="fade-error">
              <div class="invalid-feedback" v-if="formErrors.gambarutama">
...
import { addHalaman, updateHalaman } from "@/services/general/website/halaman";
...
watch(
  ...
  (newData) => {
    ...
    formData.id_pengguna = currentUser.value.idpengguna;

    if (newData) {
      formData.id_pengguna = newData.id_pengguna;
...
function loadCurrentUser() {
  const fallbackUser = { id_pengguna: "", nama: "" };
  try {
    const storedUserData = localStorage.getItem("userData");
    if (storedUserData) {
      const parsed = JSON.parse(storedUserData);
      const user =
        parsed && parsed.data && parsed.data.length > 0
          ? parsed.data[0]
          : parsed;
      ...
      if (userId && userName) {
        currentUser.value = { id_pengguna: userId, nama: userName };
...
async function submitForm() {
  ...
  try {
    if (isEditMode.value) {
      const fieldId = props.fieldToEdit.id_halaman;
      data.append("_method", "PUT");
      await updateHalaman(fieldId, data);
      toast.success(`Data ${props.entityName} berhasil diperbarui`);
    } else {
      await addHalaman(data);
