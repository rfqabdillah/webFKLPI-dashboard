<template>
  <BaseTable
    ...
    :FormModalComponent="LinkTerkaitFormModal"
    :columns="columns"
    primaryKey="id_link_terkait"
  ...
  >
    <template #filters="{ filters }">
      <div class="row g-3">
        <div class="col-md-4">
          <label for="filterName" class="form-label text-dark fw-semibold"
            >Nama Link</label
          >
          <input
            type="text"
...
import LinkTerkaitFormModal from "./LinkTerkaitFormModal.vue";

const RelatedLinkFormModal = defineAsyncComponent(() =>
  import("./LinkTerkaitFormModal.vue"),
);

const relatedLinkApi = {
  get: getLinkTerkait,
  delete: deleteLinkTerkait,
};
