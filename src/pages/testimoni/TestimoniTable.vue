        ...
        <div class="table-responsive signal-table">
          <table class="table table-hover">
            ...
            <tbody>
              ...
              <template v-else>
                <tr v-for="(item, index) in itemsList" :key="item.id_testimoni">
                  <th scope="row">
                  ...
                  <td>
                    <span
                      ...
                      :class="{
                        ...
                        'bg-light text-dark': !item.status,
                      }"
                    >
                      {{ item.status || "Draft" }}
                    </span>
                  ...
                  <td>
                    <div class="btn-group">
                      ...
                      <button
                        v-if="item.status !== 'Tayang'"
                        class="btn btn-success btn-sm"
                        @click="setStatus(item, 'Tayang')"
...
import {
  ...
  updateTestimoni,
  deleteTestimoni,
} from "@/services/general/testimoni";

// Load Component Detail Modal
...
async function setStatus(item, newStatus) {
  isUpdating.value = item.id_testimoni;

  try {
    const data = new FormData();
    data.append("record[status]", newStatus);
    data.append("_method", "PUT");

    await updateTestimoni(item.id_testimoni, data);

    // Update local state
    const index = itemsList.value.findIndex(
      (t) => t.id_testimoni === item.id_testimoni
    );
    if (index !== -1) {
      itemsList.value[index].status = newStatus;
    }
